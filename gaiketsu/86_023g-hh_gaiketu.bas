...
Declare PtrSafe Sub Sleep Lib "kernel32" (ByVal dwMiliseconds As Long)  'Call Sleepで必要
...
Function fairukopi(jj As Long, fm_path As String, fm_fn As String, to_path As String, to_fn As String)

'    Application.StatusBar = True

'    jj = 1

'    Do Until Cells(jj, 1).Value = "項目"
'        If jj = 2000 Then
'            MsgBox "項目名が無いです"
'            Exit Sub
'        End If
...
'    Do Until Cells(jj, 1).Value = NullString

'        fm_path = Cells(jj, 4).Value
'        fm_fn = Cells(jj, 5).Value '= "Book1.xlsb"
    
'        to_path = Cells(jj, 6).Value
'        to_fn = Cells(jj, 7).Value '= "Book1" & Format(Now(), "yyyymmdd_hhmmss") & ".xlsb"

        If to_fn = "" Then to_fn = fm_fn

        If fm_path = "" Then
            'MsgBox "コピー元パスが空白です"
            fairukopi = "ng1-元パスが空白_" & Format(Now(), "yyyymmdd_hhmmss") 'ok
            
        ElseIf fm_fn = "" Then
            'MsgBox "コピー元ファイル名が空白です"
            fairukopi = "ng2-元ファイルが空白_" & Format(Now(), "yyyymmdd_hhmmss") 'ok
            
        ElseIf Len(Dir(fm_path, vbDirectory)) = 0 Then
            'MsgBox "コピー元パスが見つかりません→" & fm_path
            fairukopi = "ng3-コピー元パス無し_" & Format(Now(), "yyyymmdd_hhmmss") 'ok

        ElseIf Len(Dir(fm_path & "\" & fm_fn)) = 0 Then
            'MsgBox "コピー元ファイルがありません→" & fm_fn & fm_ext
            fairukopi = "ng4-コピー元ファイル無し_" & Format(Now(), "yyyymmdd_hhmmss") 'ok

        ElseIf to_path = "" Then
            'MsgBox "コピー先パスが空白です"
            fairukopi = "ng5-コピー先パスが空白_" & Format(Now(), "yyyymmdd_hhmmss") 'ok

        ElseIf to_fn = "" Then
            'MsgBox "コピー先ファイル名が空白です"
            fairukopi = "ng6-元ファイルが空白_" & Format(Now(), "yyyymmdd_hhmmss") 'ok
        
        ElseIf Len(Dir(to_path, vbDirectory)) = 0 Then
            'MsgBox "コピー先パスが見つかりません→" & to_path
            fairukopi = "ng7-コピー先パス無し_" & Format(Now(), "yyyymmdd_hhmmss") 'ok

        ElseIf Len(Dir(to_path & "\" & to_fn)) > 0 Then
            'MsgBox "ファイルの上書きは実施しないです→" & "\" & to_fn & to_ext
            fairukopi = "ng8-上書き実施せず_" & Format(Now(), "yyyymmdd_hhmmss") 'ok

        Else '以上チェック通過、ようやくコピー実施へ
            
            FileCopy fm_path & "\" & fm_fn, to_path & "\" & to_fn
            
            fairukopi = "ok-" & Format(Now(), "yyyymmdd_hhmmss")
        
        End If
        
                Application.StatusBar = jj & "、" & fm_fn
'    Loop
    ...
    Else '複文節数(bun)確定のためのルーチン(kg1<>"")
        For ii = 1 To 8
            Do Until rvsrz3(bfshn.Cells(sr(ii), a).Value, bni, kg1, 0) = ""
                bni = bni + 1
                If bni > 120 Then Call oshimai("", bfn, shn, 4, 2, "bni120超え" & bni)
            Loop
            bni = bni - 1
...
'◇Ａ◇単文節ここまで↑
    ...
    wsfag = True     '86_022z　新たな使い方へ（アラート立てるの一回だけにする）ｚ：こっちへ移動
...
'◆Ｄ◆複文節for（：本番実行編）ふい↓　文節毎のforがここから始まる ループ部
    ...
    'mghz　４行目に区切り文字を入れる mghzの要素生成で使用  mghz→mghz+1へ
...
'                Range(bfshn.Cells(k, mghz), bfshn.Cells(h, mghz)).NumberFormatLocal = "G/標準"    'mghz列一旦標準へ（数式入れるため）
                '↓30s86_022k
                Range(bfshn.Cells(k, mghz + 1), bfshn.Cells(h, mghz + 1)).NumberFormatLocal = "G/標準" 'mghz列一旦標準へ（数式入れるため）

'                bfshn.Cells(sr(8), mghz).FormulaR1C1 = "=" & baba & "&" & """" & """"     '←数式右端に「&""」を付加(空白セルが「0」になる対策)　30s86_020s
                '↓30s86_022k
                bfshn.Cells(sr(8), mghz + 1).FormulaR1C1 = "=" & baba & "&" & """" & """"   '←数式右端に「&""」を付加(空白セルが「0」になる対策)　30s86_020s
...
'                Call copipe(bfn, shn, sr(8), mghz, sr(8), mghz, bfn, shn, k, mghz, h, mghz, 3)  '3→FormulaR1C1
                ...
                '値に変換 '◆30s86_022jここが悪さ
'                Call copipe(bfn, shn, k, mghz, h, mghz, bfn, shn, k, mghz, h, mghz, 1)  '1→Valueコピペ　旧cpp2、-4163
                '↓30s86_022k(16ケタの数値が化ける対策)文字列として貼り付けで解決、但し、空白が昇順でてっぺんに来る副作用発生satesate変数で解決
                Range(bfshn.Cells(k, mghz), bfshn.Cells(h, mghz)).NumberFormatLocal = "@"  'mghz文字列へ
                Call copipe(bfn, shn, k, mghz + 1, h, mghz + 1, bfn, shn, k, mghz, h, mghz, 1) '1→Valueコピペ　旧cpp2、-4163
                


                 '30s86_020s
                If hrkt = 16 Then baba = "ASC(PHONETIC(R[0]C[-1]))" Else baba = "ASC(R[0]C[-1])"  '前列をコピる数式
'MsgBox baba
                ...
                '30s86_022mh mghzの「""」のセルを、空白に（ソートで上に出ないようにするため）
                ...
                Do Until satesate = h + 1
                    If bfshn.Cells(satesate, mghz).Value <> "" Then
                        If satesate > k Then
MsgBox "空白発動"
                            Range(bfshn.Cells(k, mghz), bfshn.Cells(satesate - 1, mghz)).NumberFormatLocal = "G/標準"
                            Call copipe(bfn, shn, k, mghz, satesate - 1, mghz, bfn, shn, k, mghz, satesate - 1, mghz, 1) '1→Valueコピペ　旧cpp2、-4163
                        End If
                        Exit Do
                    End If
                    satesate = satesate + 1
                ...
                Range(bfshn.Cells(k, mghz), bfshn.Cells(h, mghz)).NumberFormatLocal = "@"  'mghz文字列へ
                'ここまで30s86_022mh mghzの「""」のセルを、空白に（ソートで上に出ないようにするため）ここまで
...
'pap系の値策定はここまで。以降でpapの値はないと思われる。
    'MsgBox IsNumeric("&H" & mr8(0))
    '↓mr8(pap(7,0))→mr8(0) 86_017v
    If Len(mr8(0)) > 1 And (er(8, bni) < 0 Or Round(er(6, bni), 0) = -15 Or Round(er(6, bni), 0) = -14 Or Round(er(6, bni), 0) = -20) Then   '-20追加202401
        'If Val("&H" & mr8(0)) <> 0 Then
        '↓30s86_022r
        If IsNumeric("&H" & mr8(0)) Then
   ...
   'wsfag = True     '86_022x　新たな使い方へ（アラート立てるの一回だけにする）
...
'                am2 = hirt(ii + pqp, 1)   '30s85_027検証6
            ...
            If InStr(am2, "?") = 0 And InStr(StrConv(am2, 8 + hrkt), "?") > 0 Then
                If wsfag = True Then   '86s_022x アラート一回だけに
                    MsgBox "uniあり注意です（" & am2  '86_020s　24→8 + hrkt
                    wsfag = False
                End If
            End If
    ...
    If mr(1, 11, 1) = "純高速" Then '高速シートAーE列入れ込み戻し、当列、突合列へベタ
        ...
        If h0 < h Then  '新規あるときのみ ←h1→h0へ。準高速onlyなので　And pap(2,0)撤廃
            ...
            If (er(10, bni) = 0 Or (er(7, bni) <> 0 And er(7, bni) <> 0.1)) And er(6, bni) > 0 Then  '修正 86_023gh
            '突合列新規ベタ(単数列時のみ)
                '当列控え
...
'-99､-98用ここから
  'If er(6, bni) <= -90 And StrConv(Left(bfshn.Cells(sr(1), a).Value, 1), 8) <> "*" Then 'るC　20190214　*ありは実施しないに
    '↓30s86_022qhs
  If er(6, bni) <= -88 And StrConv(Left(bfshn.Cells(sr(1), a).Value, 1), 8) <> "*" And h >= k Then 'るC　20190214　*ありは実施しないに
        '-88：ファイルコピーツール(86_023eh)
        '独立集計(ｈ変更)
        'こちらの「ど」の対応はどうするか要検討
        ...
        '↓(86_023eh)、ファイルコピーツール時(-88)は実施しないへ
        ElseIf er(6, bni) <= -90 Then '従来型 -4123→xlPasteFormulas→.copy.paste
       ...
       'Range(bfshn.Cells(k, a), bfshn.Cells(h, a)).NumberFormatLocal = "G/標準"
        ...
        If er(5, bni) <> 1 Or er(6, bni) = -88 Then  '-88はここで実行
            Application.Cursor = xlWait
            For ii = k To h
                ...
                ElseIf er(6, bni) = -88 Then  '-88
                    bfshn.Cells(ii, a).Value = fairukopi(ii, bfshn.Cells(ii, er(2, bni)).Value, bfshn.Cells(ii, er(7, bni)).Value, bfshn.Cells(ii, er(3, bni)).Value, bfshn.Cells(ii, er(8, bni)).Value)
                End If
                Call hdrst(ii, a)            '左下ステータス表示部
            Next
            Application.Cursor = xlDefault
...
Function tnele(ct8 As String, ax As Long, qap0 As Long, ct3 As String, er78() As Currency, a As Long, hx As Long, bni As Long, qq As Long, fuk12 As Long, mr() As String, er() As Currency, mr8() As String) As String '転載エレメント決定　86_013j
    ...
    ElseIf Round(er(6, bni), 0) = -20 Then    '-20 文字列を規定文字数毎に区切る
        tnele = kugiru(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), Val(mr(2, 6, bni)), mr8(0), 0)
    ElseIf Round(er(6, bni), 0) = -16 Then 'gasのアレ(mae,usiro,naka,aida)相当  30s86_023c-dh
        If mr(2, 6, bni) = "usiro" Then '区切り文字は含まない
            If InStrRev(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)) > 0 Then
                tnele = Mid(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), 1 + InStrRev(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)))
            Else '区切り文字ない時は空白返し
                tnele = ""
            End If
        ElseIf mr(2, 6, bni) = "temae" Then '区切り文字は含まない
            If InStrRev(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)) > 0 Then
                tnele = Left(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), -1 + InStrRev(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)))
            Else '区切り文字ない時は丸ごと返し
                tnele = Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0)))
            End If
        ElseIf mr(2, 6, bni) = "naka" Then '区切り文字の間を返す（区切り文字は含まない）
            If InStr(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)) = InStrRev(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)) Then
                tnele = ""  '↑区切り文字１個のとき（空白返し）
            Else '↓区切り文字が２個以上のみが対象
                tnele = Mid(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), 1 + InStr(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)), _
                InStrRev(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)) - InStr(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)) - 1)
            End If
        ElseIf mr(2, 6, bni) = "aida" Then '区切り文字の間を返す（区切り文字を含む）
            If InStr(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)) = InStrRev(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)) Then
                tnele = ""  '↑区切り文字１個のとき（空白返し）
            Else '↓区切り文字が２個以上のみが対象
                tnele = Mid(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), InStr(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)), _
                InStrRev(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)) - InStr(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)) + 1)
            End If
        End If
...
'            am2 = CStr(Workbooks(fn).Sheets(f).Cells(ii, Abs(erx(0))).Value)  '高速時はtrim実施しないで揃えるへ　85＿027検証6
            ...
            If Left(am2, 1) = "'" Then   '30s86_022vh
                'MsgBox "↓アポス出現、除去します" & vbCrLf & am2
                am2 = Mid(am2, 2)
            End If
...
'            am2 = Trim$(CStr(Workbooks(fn).Sheets(f).Cells(ii, Abs(erx(0))).Value))
            ...
            If Left(am2, 1) = "'" Then   '30s86_022uh
                'MsgBox "↓アポス出現、除去します" & vbCrLf & am2
                am2 = Mid(am2, 2)
            End If
...
'                    am3 = Trim$(CStr(Workbooks(fn).Sheets(f).Cells(ii, Abs(erx(qap))).Value))
                ...
                If Left(am3, 1) = "'" Then   '30s86_022uh
                    'MsgBox "↓アポス出現、除去します" & vbCrLf & am3
                    am3 = Mid(am3, 2)
                End If
...
Function rvsrz3(cef As String, bni As Long, kgr As String, nkg As Long) As String '30s03
    '対象文字列(A:*ｦあｦい)、文節数、区切り文字、区切りしないフラグ
    ...
    Else 'nkg=0, nkg=2：ヲ有り（先頭＊含む）
        ...
        If cc = 0 And ipc = bni Then  'ジャスト規定文節で区切無しに　０文節もこちらに入る。
          If ipc > 1 Then '86_022s
            'rvsrz3 = Mid(cef, bb + 1)
            '↓30s86_022r
            rvsrz3 = Mid(cef, bb + Len(prd))
          
          Else '1文節目で未到達
          
'         MsgBox "cef:" & cef & "、bb:" & bb & "、" & "、kgr:" & kgr & "、" & Mid(cef, bb + 1)
'            rvsrz3 = ""
'            rvsrz3 = Mid(cef, bb + 1)
            '↓86_022s  (↑bbが0以外のケースはないと思われるので↓へ)
            rvsrz3 = cef
            If bb <> 0 Then MsgBox "bbが０以外っす"    'もしbb＜＞0のケースあれば、別途検討へ　86_022s
...
'            rvsrz3 = Mid(cef, bb + 1, cc - bb - 1)
            ...
            'rvsrz3 = Mid(cef, bb + Len(prd), cc - bb - Len(prd))
        '　　↓86_022s
          If ipc = 1 Then  '86_022s
            rvsrz3 = Mid(cef, 1, cc - bb - 1)
          Else
            rvsrz3 = Mid(cef, bb + Len(prd), cc - bb - 1)
          End If
...
End Function
'ー◆ー◆ー◆ー◆ー◆ー◆ー◆ー◆ー◆ー◆ー◆ー◆ー◆ー
Function kugiru(cef As String, bni As Long, kgr As String, nkg As Long) As String '30s86_023bh
    'cef:対象文字列、bni:区切る文字単位数、区切り文字、区切らないフラグ(0)
    'bni=0 (文節ゼロ)→そのままフルで返す
    Dim ipc As Long, bb As Long, cc As Long, prd As String
    ...
    If nkg = 1 Then
        kugiru = cef  '区切りなしフラグ有効→そのまま返す
    
    Else 'nkg=0
        If bni = 0 Then
            kugiru = cef
        Else
            kugiru = Mid(cef, 1, bni)
            Do Until Len(cef) < bni * ipc + 1
                a = 1
                kugiru = kugiru & prd & Mid(cef, bni * ipc + 1, bni)
                ipc = ipc + 1
            Loop
        End If
    End If
...
'                Sheets("高速シート_" & syutoku()).Name = shemei & "のPW"
                ...
                Cells(7, 1).Value = "ファイル名： " & fimei & "." & fmt
                Cells(8, 1).Value = "ファイルのＰＷ：" & wd
                Cells(9, 1).Value = "シート名：" & shemei
...
'                If wd <> "" Then Shell "c:\windows\system32\notepad.exe", vbNormalFocus 'PW用メモ帳立ち上げ 86_020z　廃止へ
                ...
                Else                  'MsgBox "excel2016以降です"
                    If IsNumeric(syutoku()) Then '数字のIDのPC
                        '↓レノボのPC
                    ...
                    Else 'not数字、英数字のIDのPC
                        ...
                        'x64対応202503
                        If Len(Dir("C:\Program Files (x86)\Microsoft Office\Root\Document Themes 16\Theme Fonts", vbDirectory)) = 0 Then
                            ActiveWorkbook.Theme.ThemeFontScheme.Load ( _
                            "C:\Program Files\Microsoft Office\root\Document Themes 16\Theme Fonts\Office 2007 - 2010.xml" _
                            )
...
Function tskup(am1 As String, am2 As String, n1 As Long, n2 As Long, h As Long, b As Currency, c As Currency, k0 As Long, h0 As Long, pap2 As Long, er2() As Currency, spd As String, pqp As Long, e5 As Long, er3() As Currency) As Long  'じあ
    ...
    If tskup = 0 Then 'まだ決まらず(p=0)→マッチング実施
        ...
        Else 'ノーマルor純　 am2→strcnv化　85_008
            ...
            '↓86_022th  「~」の対処
            If IsError(Application.Match(Replace(LCase(am2), "~", "~~"), Range(bfshn.Cells(k0, Abs(b)), bfshn.Cells(h0, Abs(b))), 0)) Then  '新規2
            ...
            Else
                tskup = 1
                m = Application.WorksheetFunction.Match(Replace(LCase(am2), "~", "~~"), Range(bfshn.Cells(k0, Abs(b)), bfshn.Cells(h0, Abs(b))), 0)  '完全一致
                If spd = "純高速" Then '高速時
                    MsgBox "こちらは低速専用になりました。純高速でこちら通るのはおかしい。"
                Else 'not高速時
                    n2 = k0 + m - 1
