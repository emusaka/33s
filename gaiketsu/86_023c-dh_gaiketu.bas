...
Sub fairukopi()
    ...
    Do Until Cells(jj, 1).Value = NullString
        ...
        If fm_path = "" Then
            MsgBox "コピー元パスが空白です"
            Cells(jj, 3).Value = "ng1-元パスが空白_" & Format(Now(), "yyyymmdd_hhmmss") 'ok
            
        ElseIf fm_fn = "" Then
            MsgBox "コピー元ファイル名が空白です"
            Cells(jj, 3).Value = "ng2-元ファイルが空白_" & Format(Now(), "yyyymmdd_hhmmss") 'ok
            
        ElseIf Len(Dir(fm_path, vbDirectory)) = 0 Then
            MsgBox "コピー元パスが見つかりません→" & fm_path
            Cells(jj, 3).Value = "ng3-コピー元パス無し_" & Format(Now(), "yyyymmdd_hhmmss") 'ok

        ElseIf Len(Dir(fm_path & "\" & fm_fn)) = 0 Then
            'MsgBox "コピー元ファイルがありません→" & fm_fn & fm_ext
            Cells(jj, 3).Value = "ng4-コピー元ファイル無し_" & Format(Now(), "yyyymmdd_hhmmss") 'ok

        ElseIf to_path = "" Then
            MsgBox "コピー先パスが空白です"
            Cells(jj, 3).Value = "ng5-コピー先パスが空白_" & Format(Now(), "yyyymmdd_hhmmss") 'ok

        ElseIf to_fn = "" Then
            MsgBox "コピー先ファイル名が空白です"
            Cells(jj, 3).Value = "ng6-元ファイルが空白_" & Format(Now(), "yyyymmdd_hhmmss") 'ok
        
        ElseIf Len(Dir(to_path, vbDirectory)) = 0 Then
            MsgBox "コピー先パスが見つかりません→" & to_path
            Cells(jj, 3).Value = "ng7-コピー先パス無し_" & Format(Now(), "yyyymmdd_hhmmss") 'ok

        ElseIf Len(Dir(to_path & "\" & to_fn)) > 0 Then
            'MsgBox "ファイルの上書きは実施しないです→" & "\" & to_fn & to_ext
            Cells(jj, 3).Value = "ng8-上書き実施せず_" & Format(Now(), "yyyymmdd_hhmmss") 'ok

        Else '以上チェック通過、ようやくコピー実施へ
            
            FileCopy fm_path & "\" & fm_fn, to_path & "\" & to_fn
            
            Cells(jj, 3).Value = "ok-" & Format(Now(), "yyyymmdd_hhmmss")
        
        End If
        
        jj = jj + 1
                Application.StatusBar = jj
    Loop
