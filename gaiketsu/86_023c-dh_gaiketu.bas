...
Function tnele(ct8 As String, ax As Long, qap0 As Long, ct3 As String, er78() As Currency, a As Long, hx As Long, bni As Long, qq As Long, fuk12 As Long, mr() As String, er() As Currency, mr8() As String) As String '転載エレメント決定　86_013j
    ...
    ElseIf Round(er(6, bni), 0) = -16 Then 'gasのアレ(mae,usiro,naka,aida)相当  30s86_023c-dh
        If mr(2, 6, bni) = "usiro" Then
            If InStr(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)) > 0 Then
                tnele = Mid(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), 1 + InStr(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)))
            Else
                tnele = ""
            End If
        ElseIf mr(2, 6, bni) = "temae" Then
            If InStr(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)) > 0 Then
                tnele = Left(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), -1 + InStr(Workbooks(mr(2, 1, bni)).Sheets(mr(1, 1, bni)).Cells(qq, Abs(er78(1, qap0))), mr8(0)))
            Else
